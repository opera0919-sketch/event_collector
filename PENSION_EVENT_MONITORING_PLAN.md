...
> 작성일: 2026-06-12 / 상태: **검토 완료 (구현 가능성 검증 반영)** / 버전: v1.1
>
> v1.0 초안 작성 후 실행 환경에서 구현 가능성을 직접 검증하고, 재검토에서 발굴한
> 개선 사항을 반영한 버전입니다. 검증 결과는 §2, 개선 사항은 §9에 정리했습니다.
...
| 항목 | 내용 |
|---|---|
| 대상 증권사 | 미래에셋증권, 한국투자증권, 삼성증권, 키움증권, KB증권, NH투자증권 |
...
### 5.1 수집 대상 페이지 (1차 후보 — Phase 1에서 확정)

| 증권사 | 이벤트 목록 페이지 (후보) | 비고 |
|---|---|---|
| 미래에셋증권 | `securities.miraeasset.com/mw/mki/mki7000/r01.do` | 진행중 이벤트 목록 (검색으로 실존 확인) |
| 한국투자증권 | `securities.koreainvestment.com` 이벤트 메뉴 | URL 확정 필요 |
| 삼성증권 | `www.samsungpop.com` 이벤트 메뉴 | MENU_CODE 방식, URL 확정 필요 |
| 키움증권 | `www1.kiwoom.com/h/customer/event/VIngEventView` | 진행중 이벤트 (검색으로 실존 확인) |
| KB증권 | `www.kbsec.com` 이벤트 메뉴 | URL 확정 필요 |
| NH투자증권 | `www.nhqv.com` 이벤트 메뉴 | URL 확정 필요 |
...
## 10. 사용자 결정 필요 사항 (Phase 0)

1. **메일 발송 수단**: A안(Gmail SMTP + 앱 비밀번호) 권장 — 동의 여부.
   동의 시 Gmail 앱 비밀번호를 GitHub Secrets(`MAIL_APP_PASSWORD`)로 등록 필요.
2. **수신 주소**: `opera0919@gmail.com` 단일 수신으로 충분한지.
3. **해외 IP 차단 대비**(§9-③): Phase 1 실측에서 차단 증권사가 나올 경우,
   세션 네트워크 정책에 증권사 도메인 허용을 추가할 의향이 있는지.
4. 리포트에 **종료 임박(7일 이내) 이벤트 알림** 섹션 추가 여부 (권장: 추가).
