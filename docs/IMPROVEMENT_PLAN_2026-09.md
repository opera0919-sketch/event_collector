...
| **P1-lite** — *PR 반영(KB 보완)* | (a) 이미지 선택을 **폴더 단위 전량·자연 정렬**로 교체(`select_content_images`, `MAX_IMAGES=12`) — KB 슬라이스 img_01…img_11 을 매일 다른 3장이 아니라 전부 읽음 (b) 같은 실행의 동일 증권사 상세 페이지 공통 라인 제거본으로 재추출 해시 계산(`hash_text_per_firm`, LLM 입력은 불변) (c) **사실 시그니처 게이트**(`facts.py`, `db.material_change`) — 수치·티어·판정·배수가 같거나 티어가 줄면 기존 캐노니컬·타입드·자식 행 유지, 변경 로그 없음 (d) 워크플로 커밋 단계: 산출물 보관 → 원격 최신 위에 재적용 → 푸시 3회 재시도, 실패 시 잡 실패 표시 | KB 해시 flip·티어 누락 제거, 표현 요동 무변경 | 첫 실행은 해시식 변경으로 전건 1회 재추출(스키마 bump 와 동일) |
| **P1 (잔여)** | S2 컨테이너 셀렉터 해시(미래·삼성·NH·한투) + `event_snapshots`/`event_extractions`, 리포트 변경 섹션을 시그니처 diff 로, 관측 컬럼 | 변경 로그 ≤2/일, 캐시 적중 ≥90% | 컨테이너 셀렉터 오판 시 본문 누락 → 스냅샷 텍스트 길이 하한 경보로 방어 |
