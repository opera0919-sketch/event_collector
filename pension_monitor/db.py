...
# 콘텐츠 '실질 변경' 판정용 금액/수치 시그니처 — 금액·문턱·티어·%·인원 등 경제적
# 실체만 뽑아 비교한다. OCR 재추출은 상세 본문의 휘발 요소로 매 실행 캐시가 헛돌아
# 재수행되는데(운영 관측), LLM 이 문구(및→후, 어순, '상품권' 접두 등)만 바꾸고 수치는
# 그대로면 시그니처가 같아 '변경'으로 기록하지 않는다 → 리포트/메일의 가짜 '변동' 노이즈
# 차단. 마스터 값(updates[f])은 그대로 최신화되므로 현재 데이터는 항상 정확하며,
# 억제되는 것은 '직전 대비 변동' 알림뿐이다(마스터/자식 불일치 없음).
_AMT_TOKEN_RE = re.compile(
    r"(\d[\d,\.]*)\s*(백만원|만원|천원|원|만|천|%|명|회|주|건|배|개월|일)?")


def _amount_sig(text) -> tuple:
    """텍스트의 금액/수치 토큰 멀티셋(정렬 튜플). 쉼표 제거·단위 보존."""
    out = []
    for m in _AMT_TOKEN_RE.finditer(text or ""):
        num = m.group(1).replace(",", "").rstrip(".")
        if num:
            out.append(num + (m.group(2) or ""))
    ...
    for ev in scraped:
        ...
        if old is None:
            ...
            for f in _CONTENT_FIELDS:
                if (old.get(f) or None) != (ev.get(f) or None):
                    updates[f] = ev.get(f)   # 마스터는 항상 최신화 (현재 데이터 정확성)
                    # '변경' 기록은 실질(금액/수치)이 바뀐 경우에만 — benefits/conditions
                    # 의 문구만 다른 재추출은 노이즈라 스킵. acct_etc 는 짧은 코드값이라
                    # 정밀 비교, 이전이 비어 있던(새로 채워진) 경우도 실질로 본다.
                    material = (f == "acct_etc"
                                or not str(old.get(f) or "").strip()
                                or _amount_sig(old.get(f)) != _amount_sig(ev.get(f)))
                    if ev.get("rows_fresh") and material:
                        changed.append((ev, f, old.get(f), ev.get(f)))
