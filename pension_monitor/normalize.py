...
from .classify import (
    ...
    hold_period, source_content_hash, select_content_images, _TRANSFER_HINT, _MULT_HINT,
)
...
def _resolve_banner_images(ev):
    ...
    try:
        ...
        cands = [urljoin(url, img.get("src") or img.get("data-src") or img.get("data-original") or "")
                 for img in soup.find_all("img")]
        # 세 수집 경로 공통 규칙(폴더 단위 전량·자연 정렬) — 실행마다 다른 이미지를 집던 churn 제거
        return select_content_images(cands, vision.MAX_IMAGES)
